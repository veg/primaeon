...
| `datamonkey3/static/wasm/`, `NJ.bf`, `stores/aioli.js` | ~~HyPhy WASM + NJ~~ | used in Phases 1–2, removed in Phase 3 (D22) |
...
| **3 — Phenotype in the browser, tree-free TN93, bridge gone** (≈3 weeks) | Port 5 (phenotype, permulations, busted neural head); trait UI and plots; **TN93 tree-free mode ported to the library and made the default when a usable tree is absent, JS neighbour-joining for display, HyPhy WASM and its vendored build removed (D22)**; statistical parity for permutation outputs; Python bridge deleted; four-way parity in CI | No Python and no HyPhy at runtime anywhere in the product; all five examples fully browsable and at parity, camelid and HIV-1 RT included |
...
| D22 | Tree preparation without HyPhy | **Resolved (user direction, 2026-09-05): tree-free mode with TN93.** HyPhy WASM was in the app only to fit HKY85 branch lengths or build an NJ tree when the input lacked them, mirroring what `dataset.py` does by shelling out to `hyphy`. This app is for HyphAeon: the reference's own `--use-tn93` path (pairwise TN93 distances straight into the MDS) is ported to the library and becomes the default whenever a usable tree is absent; a small JS neighbour-joining routine on the same distances provides a topology for display only. HyPhy WASM, its vendored build and the HBL scripts are removed in Phase 3. The remaining parity gap on camelid and HIV-1 RT (WASM HyPhy 2.5.98 vs native 2.5.65) disappears with it, since Python and the library then compute the same TN93 matrix. |
