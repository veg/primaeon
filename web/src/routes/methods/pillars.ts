...
export const PILLARS: readonly Pillar[] = [
	{
		...
		validated:
			'The library’s diagnose() is the same code in the browser worker, the MCP’s hyphaeon_validate and the server’s /validate, so the codes cannot drift between surfaces. The tree-free distances are the reference’s own, computed by veg/tn93’s compiled code — the only TN93 in the product, vendored as WebAssembly and sha256-verified before it runs — and its matrix is bit-identical to the Python package’s on every bundled example, with the MDS coordinates that follow agreeing to 7.8e-8 without any sign allowance. Everything applied to your data is shown in the strip at the top of the report and can be changed under “Re-run with”.',
		real: null
	},
	{
		id: 'sites',
