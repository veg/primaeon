<!--
	...
	transports and the tool set; the native/bridged split follows mcp/src/validate.js
	(NATIVE_ANALYSES / BRIDGED_ANALYSES) and the provenance.surface values the server writes. The
	transcript is data in ./transcript.ts, recorded against the stdio server. The snippet
...
<script lang="ts">
	...
	type Runs = 'native' | 'bridged' | 'control';
	interface Tool {
		name: string;
		returns: string;
		runs: Runs;
	...
	const tools: readonly Tool[] = [
		{
			...
			returns:
				'The whole report: diagnostics, sites, gene, epistasis and sectors, attribution, filter, DMS (progressive, capped), provenance and timings, in the schema the web report reads.',
			runs: 'native',
			note: 'The same options as the report\'s "Re-run with": model_variant, max_species, reference_sequence, call_mode, seed, permutations, dms.'
		},
		...
		{ name: 'hyphaeon_epistasis', returns: 'Co-selection edges with CESI and q, sectors with coherence and p_perm, optional per-sector DMS, GraphML.', runs: 'bridged', note: 'Ported in phase-2a; the in-process switch follows.' },
		{ name: 'hyphaeon_dms', returns: '19-substitution scan per site with intrinsic plasticity and the ΔLRT map.', runs: 'bridged', note: 'Ported in phase-2a; the in-process switch follows.' },
		{ name: 'hyphaeon_phenotype', returns: 'Trait association per site, PARS signature, trait sectors, permulation p.', runs: 'bridged' },
		...
		{ name: 'list_models', returns: 'The weights manifest, the engine\'s status, and whether the Python bridge is reachable.', runs: 'control' }
	];

	const RUNS_LABEL: Record<Runs, string> = {
		native: 'in-process',
		bridged: 'Python bridge',
		control: '—'
	};
...
<div class="container container--narrow">
	...
	<section>
		...
		<dl class="env">
			...
			<dt><code>HYPHAEON_PY_BIN</code></dt>
			<dd>the Python <code>hyphaeon</code> CLI, needed only for the bridged tools below.</dd>
		</dl>
	</section>
	...
	<section>
		...
		<p>
			...
			result matches the reference at the published parity classes. <strong>Python bridge</strong>
			means the tool shells to <code>hyphaeon &lt;cmd&gt;</code> with the same schema and result
			shape and writes <code>provenance.surface: "python-reference"</code>; it needs the Python
			package installed and disappears pillar by pillar as the port lands.
		</p>
		<div class="scroll">
			<table class="tools">
				<thead>
		...
		<p>
			...
			(your alignment, tree or options) or <code>server</code> (the model files, the runtime,
	...
	<section>
		...
		<ul class="convention">
			...
			<li>A tree fitted by HyPhy in the browser is a <code>file://</code> tree in the snippet, because the estimated tree is one of the report's downloads.</li>
